...
func main() {
	...
	mux.HandleFunc("/ui", func(w http.ResponseWriter, _ *http.Request) {
		...
		_, _ = w.Write([]byte(`<!doctype html><html><head><meta charset="utf-8"><title>Agentic</title>
...
<body>
<h1>Agentic</h1>
<p>Extracted process plugin. Host proxies <code>/p/` + id + `/</code> after auth. Enable via Control Bus — no gateway restart.</p>
<p id="st">checking…</p>
<script>
...
.catch(e=>{document.getElementById("st").textContent="API error: "+e;});
	...
	log.Printf("agentic plugin id=%s", id)
